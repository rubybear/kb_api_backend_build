...
@workout_router.post("/exercises/create", response_description="Add new exercise", response_model=WorkoutModel)
async def create_exercises(request: Request, user: User = Depends(get_user_db),
    ...
    new_exercises = await user_db.db["exercises"].insert_one(exercises)
    inserted_id = new_exercises.inserted_id
    created_exercises = await user_db.db["exercises"].find_one({"_id": inserted_id})
